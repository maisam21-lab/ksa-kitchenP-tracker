...
def main():
    ...
    if section == "Dashboard":
        ...
        chosen_label = st.selectbox(
            ...
            key="dash_source",
            help="Select which tab's data to use. You can filter and download it below.",
        )
        ...
        if not rows:
            ...
            if is_tracker:
                st.subheader("Trends (main tracker)")
                if HAS_EXCEL:
                ...
                st.subheader("Export full summary (HTML)")
            ...
            with st.expander("How this Dashboard works", expanded=False):
                st.markdown("""
                - **Data source:** Choose any tab (main tracker, Execution Log, SF Kitchen Data, etc.) to build your report from.
                - **Refine your data:** Search in all columns; for main tracker you can also filter by date, site, region, metric in the expander.
                - **Download my report (CSV):** Export the current filtered table. Save/load views for main tracker only.
                - **Trends** and **HTML summary** appear when main tracker is selected. Use **Data** in the sidebar to import or edit.
                """)
