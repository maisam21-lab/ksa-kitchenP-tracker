...
-- =============================================================================
-- Source: sf_kitchens + sf_facilities + sf_accounts + sf_opportunities (one opportunity per facility, by oldest churn_date).
-- One row per kitchen (~999). Each kitchen linked to the oldest opportunity only.
...
SELECT
  ...
  Kitch.currency_code                                  AS currency_code,
...
LEFT JOIN (
  ...
  QUALIFY ROW_NUMBER() OVER (PARTITION BY facility_id ORDER BY churn_date ASC NULLS LAST) = 1
